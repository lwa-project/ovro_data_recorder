...
def _launch_mp_storagelogger(quota_mode, log, id, directory, quota=None, shutdown_event=None, update_interval=3600):
    """
    ...
    DiskStorageLogger/TimeStorageLogger with the exception of `log` which is
    expected to be a filename for the logfile or `None` for `sys.stdout`.
    """
    ...
    if quota_mode == 'time':
        sl = TimeStorageLogger(ilog, id, directory, quota=quota, update_interval=update_interval)
    else:
        sl = DiskStorageLogger(ilog, id, directory, quota=quota, update_interval=update_interval)
    sl.main()
